...
foreach ($array_data as $slug => $data) { 
  ...
  foreach ($data['datasample'] as $key => $array_data) {
    ...
    if ( $array_data['tx_data_unit'] === $data_unit) {
      ...
      // echo "algo esta mal"; return false;
    }
  }
  $array_vehicule[$slug]['name'] = $data['vehicule'];
  if ($distance_sum === 0 && $time_sum === 0) {
    # code...
  }
...
// echo json_encode($array_vehicule);
  ...
  foreach ($array_vehicule as $slug => $vehicule_data) {
    ...
    for ($i=0; $i < count($volume_xaxis); $i++) { 
    ...
    for ($i=0; $i < count($currency_xaxis); $i++) { 
...
<?php
  ...
  var chart_distancexvolume = c3.generate({ bindto: '#distancexvolume_chart',
    data: { columns: distancexvolume_chartdata,  type: 'bar',  labels: true, },
    bar: {  width: {  ratio: 0.8  } },
    tooltip: { show: true },
  });
  ...
  var chart_timexvolume = c3.generate({ bindto: '#timexvolume_chart',
    data: { columns: timexvolume_chartdata,  type: 'bar',  labels: true, },
    bar: {  width: {  ratio: 0.8  } },
    tooltip: { show: true },
  });
  ...
  var chart_currencyxvolume = c3.generate({ bindto: '#currencyxvolume_chart',
    data: { columns: currencyxvolume_chartdata,  type: 'bar',  labels: true, },
    bar: {  width: {  ratio: 0.8  } },
    tooltip: { show: true },
  });
